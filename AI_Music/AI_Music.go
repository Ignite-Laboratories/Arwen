...
func (sys *ai_musicSystem) GetBinaryData(entity JanOS.Entity) (BinaryData, bool) {
	...
	asset, err := JanOS.Universe.Assets.LoadAsset(assetName, fmt.Sprintf("Training/AI_Music/%s", p.Path))
	if err != nil {
		panic(err)
	}
...
func (sys *ai_musicSystem) Initialize() {
	...
	metadataAsset, err := JanOS.Universe.Assets.LoadAsset(metadataName, "Training/AI_Music/audio.csv")
	if err != nil {
